...
def count_the(l: list) -> int:
    count = 0            # create, assignment (2)
    for s in l:          # repeat: len(l)
        if s == 'the':   # compare strings (3)
            count += 1   # compute the add, then assignment
    return count         # 1 to return
# worst case, best case: ~ len(l) * C
...
def is_member(l: list, ele) -> bool:
    for x in l:          # where is ele? if at all?
        if x == ele:     # 1 comparison
            return True  # constant
    return False         # constant
# worst case: element isn't there (or at the end)
#     "linear in length of input"   O(len(l))
# best case: element is there, right at beginning!
#     "constant in length of input" O(1)
...
def distinct(the_list: list) -> list:
    seen = []
    for element in the_list:      # len(the_list)
        if element not in seen:   # len(seen)
            seen.append(element)  # suppose 1
    return seen
# worst case:
# best case: all words same, so len(seen) <= 1; O(n)
